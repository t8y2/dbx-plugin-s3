<script>
  ...
  let { selected = null, preview, text, onSheetChange } = $props();
...
<div class="preview-panel">
  ...
  {:else}
    <div class="preview-title"><strong>{selected.name}</strong><small>{preview.type}{preview.truncated ? ` · ${text.truncated}` : ""}</small></div>
...
<style>
  ...
  .preview-title { display: flex; align-items: center; justify-content: space-between; gap: 12px; height: 34px; min-height: 34px; padding: 0 16px; border-bottom: 1px solid color-mix(in srgb, CanvasText 12%, transparent); }
  .preview-title strong { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 14px; font-weight: 600; }
